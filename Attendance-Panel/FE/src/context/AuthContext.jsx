...
export const AuthProvider = ({ children }) => {
  ...
  // On component mount, check for stored user
  useEffect(() => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      try {
  ...
  const login = async (email, password, rememberMe, loginMode) => {
    try {
      ...
      if (!passwordStr) throw new Error("Password is required");
      ...
      const endpoint =
        loginMode === "admin"
          ? `http://40.192.64.65/api/auth/login/admin/`
          : `http://40.192.64.65/api/auth/login/user/`;
